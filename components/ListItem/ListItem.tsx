...
const getValue = (item: PurchaseEntity | TransactionEntity | IncomeEntity): String => {
  let value = item.amount;

  if (item.entity === ExpenseEnum.Purchase && !item.isRefund) {
    value = -value;
  } else if (item.entity === ExpenseEnum.Transaction && item.transactionType === TransactionOperation.SENT) {
    value = -value;
  }
...
export const CustomListItem = ({ item, options, label, onPress = () => {}, onLongPress = () => {}, selected = [] }: ListItemProps) => {
  const styles = _styles;
  let iconComponent;

  const loadIcon = (data: PurchaseEntity | TransactionEntity | IncomeEntity) => {
    if (data.entity === ExpenseEnum.Income) return utilIcons(23).find((category) => category.label === data.entity);
    else if (data.entity === ExpenseEnum.Purchase) return categoryIcons(30).find((category) => category.label === data.type);
  ...
  return (
    <Pressable
      key={`${item}`}
      style={({ pressed }) => [commonStyles.onPressBounce(pressed, styles.button, onPress, 12), { backgroundColor: selected.includes(item) ? dark.secundary : "transparent" }]}
      ...
      <View style={styles.rowGap}>
            ...
            <Text
              style={{
                ...
                color:
                  ...
                  (item?.entity == ExpenseEnum.Transaction && item.transactionType == TransactionOperation.RECEIVED) ||
        ...
        <View style={{ ...styles.row, flex: 1 }}>
          <View style={styles.optionsContainer}>
            ...
            {options.map((option) => (
              <Pressable key={`${item.id}${option.type}`} style={styles.optionBox} onPress={option.callback}>
